...
- **DB:** Supabase (same project as GradeX or your own; run migrations in `supabase/migrations/`)
...
3. **Set environment variables** in the Railway service:
   ...
   - **Optional:** `PUBLIC_URL` — set to your Railway app URL (e.g. `https://certvault-production.up.railway.app`) if CORS blocks requests; Railway may also set `RAILWAY_PUBLIC_DOMAIN`, which is used automatically.
   - **Optional:** `CERTGEN_SERVICE_URL` — only if you run a separate PDF generator service; omit to use CertVault without PDF generation.

4. **Generate a domain** in Railway (Settings → Networking → Generate Domain). Use this URL as `PUBLIC_URL` if needed.

5. Run **Supabase migrations** (e.g. from `supabase/migrations/`) against your Supabase project so tables exist.
...
Supabase tables are in `supabase/migrations/` (042, 043, 044). Run them against your Supabase project (same as GradeX or a new project).
